...
creer la manette virtuelle change :
    ...
    Linux   : evdev / uinput      (STK_input_server_v2.py)
...
peut moduler la touche (appuis rythmes, voir COLLABORATIF/modulation.py) et ca
aide beaucoup, mais ca reste une suite d'a-coups.
...
Le pilote ViGEmBus doit etre installe (il l'etait deja sur cette machine).

Configuration du jeu
--------------------
Options > Controles : assigner UNIQUEMENT "tourner a gauche / a droite" a l'axe
du Xbox 360 Controller, et laisser tout le reste au clavier. C'est la
configuration utilisee par l'equipe sous Linux.

Original : Michael ORTEGA - 09 jan 2018, via stk_server_maintien.py
"""
...
# Appui bref mais NON NUL. Mesure du projet (tools/test_touche.py) : un appui de
# 8 ms n'est jamais vu par le jeu, un appui de 16 ms -- une image a 60 fps --
# l'est toujours. 50 ms laissent trois images de marge.
MAINTIEN = 0.05
...
class DirectionModulee:
    ...
    Meme principe que COLLABORATIF/modulation.py, recopie ici en une vingtaine
    de lignes plutot qu'importe : le serveur doit pouvoir tourner seul, y
    compris chez quelqu'un qui n'a que ce fichier.
    """
