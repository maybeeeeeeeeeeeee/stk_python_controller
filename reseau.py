#!/usr/bin/env python3
"""L'IP du PC a saisir dans l'appli du telephone, et les couleurs de la console.

Le telephone a besoin de l'IP du PC. Un PC en a souvent plusieurs, dont des
cartes virtuelles (VMware, VirtualBox) vers lesquelles le telephone ne peut
rien envoyer : on les affiche donc avec le nom de leur carte, pour que le bon
choix saute aux yeux.
"""
...
def activer_ansi():
    """Sans ca, la vieille console Windows affiche les codes couleur en clair."""
    try:
        import ctypes
...
def noms_interfaces():
    ...
    try:
        ...
        return {}
    # ipconfig ecrit dans la page de codes OEM (cp850 en France), pas dans
    # l'ANSI du systeme : decoder avec l'un donne des accents casses.
    try:
    ...
    for ligne in texte.splitlines():
        if ligne and not ligne[0].isspace():
            # ligne de titre : "Carte reseau sans fil Wi-Fi :"
            # (le point-virgule francais est precede d'une espace insecable)
...
def bandeau_reseau(ips, port):
    """L'IP et le port a saisir dans l'application du telephone."""
    if not ips:
